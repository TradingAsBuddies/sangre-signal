"""Perplexity AI-powered formatter for stock analysis.

This module provides natural language formatting of stock analysis
using Perplexity AI, with support for English and Spanish output.
"""
...
from ..config import PERPLEXITY_CONFIG, PerplexityConfig
...
class PerplexityFormatter(BaseFormatter):
    """Formatter that uses Perplexity AI to generate natural language analysis.

    This formatter sends stock data to Perplexity and receives a well-formatted,
    plain-language analysis with risk explanations in English or Spanish.
    """

    def __init__(self, language: Literal["en", "es"] = None, config: PerplexityConfig = None):
        """Initialize the Perplexity formatter.

        Args:
            language: Output language ('en' or 'es'). Defaults to config setting.
            config: Perplexity configuration. Defaults to PERPLEXITY_CONFIG.
        """
    ...
    def _get_client(self):
        """Get or create the OpenAI-compatible client for Perplexity."""
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "The 'openai' package is required for Perplexity formatting. "
    ...
    def _call_perplexity(self, prompt: str) -> str:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": SYSTEM_PROMPTS[self.language]},
                {"role": "user", "content": prompt}
            ]
        )

        # Strip emojis and handle Windows console encoding
        text = _strip_emojis(response.choices[0].message.content)
    ...
    def _format_fallback(
        ...
        """Provide fallback formatting when Perplexity is unavailable."""
        lines = []
        divider = "=" * 60
    ...
    def format(
        ...
        """Format stock data using Perplexity AI.
        ...
        Returns:
            Perplexity-generated analysis string
        """
        ...
        try:
            ...
            analysis = self._call_perplexity(prompt)
            ...
            if self.language == "es":
                header = f"{divider}\n  ANALISIS DE {stock_info.ticker} (Potenciado por Perplexity AI)  \n{divider}"
                footer = f"\n{divider}\nGenerado: {timestamp}\n{divider}"
            else:
                header = f"{divider}\n  {stock_info.ticker} ANALYSIS (Powered by Perplexity AI)  \n{divider}"
                footer = f"\n{divider}\nGenerated: {timestamp}\n{divider}"

            return _safe_encode_for_console(f"{header}\n\n{analysis}{footer}")

        except Exception as e:
            logger.error(f"Perplexity API error for {stock_info.ticker}: {e}")
            return self._format_fallback(
                stock_info, risk_analysis, float_threshold, vix_value,
                str(e)
            )
    ...
    def format_batch(
        ...
        # Add portfolio summary if multiple successful results and Perplexity is available
    ...
    def _format_portfolio_summary(
        ...
        """Generate a portfolio-level summary using Perplexity."""
        ...
        summary = self._call_perplexity(prompt)
